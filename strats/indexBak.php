...
if(membre){
    $rStrats = $msqli->query("SELECT s.*, pick.hero as pickHero, pick.obligatoire as pickHeroOblige,
        ban.hero as banHero, ban.obligatoire as banHeroOblige,
        item.id_item as item, item.obligatoire as itemOblige,
        cm.membre_id as commAuteur, cm.commentaire as commentaire, cm.dt_creation as commDate
        FROM strats s LEFT JOIN strats_pick pick ON s.id = pick.id_strats
        LEFT JOIN strats_ban ban ON s.id = ban.id_strats
        LEFT JOIN strats_items item ON s.id = item.id_strats
        LEFT JOIN strats_comm cm ON s.id = cm.id_strats");
    $strats=array();
    if($rStrats->num_rows>0){
        while($row=$rStrats->fetch_assoc()){
            if(empty($strats[$row['id']])){
                $strats[$row['id']]=array("nom" => $row['nom'], "forces_timings" => $row['forces_timings']
                    , "faiblesses_solutions" => $row['faiblesses_solutions'], "description" => $row['description']
                    , "ft" => $row['forces_timings'], "control" => $row['control'], "massive" => $row['massive']
                    , "nb_games" => $row['nb_games'], "nb_victoires" => $row['nb_victoires']
                    , "dt_creation" => $row['dt_creation']);
            }
            if(empty($strats[$row['id']]["picks"][$row['pickHero']]) && !empty($row['pickHero'])){
                $strats[$row['id']]["picks"][$row['pickHero']]=array("oblige" => $row['pickHeroOblige']);
            }
            if(empty($strats[$row['id']]["bans"][$row['banHero']]) && !empty($row['banHero'])){
                $strats[$row['id']]["bans"][$row['banHero']]=array("oblige" => $row['banHeroOblige']);
            }
            if(empty($strats[$row['id']]["items"][$row['item']]) && !empty($row['item'])){
                $strats[$row['id']]["items"][$row['item']]=array("oblige" => $row['itemOblige']);
            }
            if(empty($strats[$row['id']]["comms"][$row['commAuteur']]) && !empty($row['commAuteur'])){
                $strats[$row['id']]["comms"][$row['commAuteur']]=array("commentaire" => $row['commentaire'], "commDate" => $row['commDate']);
            }
        }
    }
...
<body>
    ...
    <?php echo "memory=".memory_get_usage() ?>
